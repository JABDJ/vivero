...
export default function Facturas() {
    ...
    const [facturaGuardada, setFacturaGuardada] = useState(null)  // { factura, items }
    ...
    const handleGuardar = async () => {
        ...
        try {
            ...
            // Guardar para el botón PDF
            setFacturaGuardada(factura)
            setItemsGuardados(itemsToInsert)
            setSuccess(true)
            setClienteNombre('')
            setItems([])
        } catch (err) {
                        ...
                        <div className="bg-surface/50 backdrop-blur-md rounded-2xl border border-white/5 p-6 shadow-2xl flex flex-col">
                            ...
                            {success && facturaGuardada ? (
                                <div className="mt-4 space-y-3">
                                    ...
                                    <button
                                        onClick={() => { setSuccess(false); setFacturaGuardada(null); setItemsGuardados([]) }}
                                        className="w-full py-2.5 rounded-xl font-medium text-gray-400 text-sm
                                            border border-gray-700 hover:border-gray-500 hover:text-white transition-all"
                                    >
                                        + Nueva Factura
                                    </button>
                                </div>
                            ) : (
                                /* Botón guardar (solo mientras no se ha guardado) */
                                <button
